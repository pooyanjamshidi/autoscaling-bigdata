function [latency,throughput_10]=get_component_metrics(deployment_id,component_id,window)
...
if strcmp(component_stats.componentType,'spout')
    ...
    throughput_10=component_stats.spoutSummary(window_index).transferred/str2num(component_stats.spoutSummary(window_index).window); 
elseif strcmp(component_stats.componentType,'bolt')
    latency=component_stats.boltStats(window_index).executeLatency;
    throughput_10=component_stats.boltStats(window_index).executed/str2num(component_stats.boltStats(window_index).window);
end
